...
def evidence_page():
    ...
    users = execute_query(
        "SELECT user_id, username FROM system_users WHERE account_locked = 0"
    )

    # Get cases for evidence registration
    clinical_cases = execute_query(
        "SELECT case_id, reference_no FROM clinical_examinations ORDER BY case_id DESC LIMIT 50"
    )
    autopsy_cases = execute_query(
        "SELECT case_id, pm_serial_no FROM postmortem_investigations ORDER BY case_id DESC LIMIT 50"
    )
...
def transfer_evidence():
    try:
        call_procedure(
            ...
            (
                request.form.get("evidence_id"),
                session.get("user_id"),
                request.form.get("to_user_id"),
...
def chain_of_custody(evidence_id):
    evidence = execute_query(
        "SELECT * FROM physical_evidence WHERE evidence_id = %s",
        (evidence_id,),
        fetch_all=False,
    )
    logs = execute_query(
