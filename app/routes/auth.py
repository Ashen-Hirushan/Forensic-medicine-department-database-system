...
def profile():
    ...
    if request.method == "POST":
        ...
        user = execute_query(
            "SELECT password_hash FROM system_users WHERE user_id = %s",
            (user_id,),
            fetch_all=False,
        )
        if user and check_password(current_password, user["password_hash"]):
            ...
            execute_query(
                "UPDATE system_users SET password_hash = %s WHERE user_id = %s",
                (new_hash, user_id),
            )
...
def upload_picture():
    ...
    if file:
        ...
        # Update DB
        execute_query(
            "UPDATE system_users SET profile_picture = %s WHERE user_id = %s",
