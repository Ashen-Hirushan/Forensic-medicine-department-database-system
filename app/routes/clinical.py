...
def list_clinical():
    ...
    return render_template(
        "clinical/list.html", cases=cases, doctors=doctors, doctor_filter=doctor_filter
    )
...
def new_clinical():
    ...
    facilities = execute_query(
        "SELECT facility_id, facility_name FROM healthcare_facilities"
    )
    ...
    return render_template(
        "clinical/form.html", facilities=facilities, stations=stations, doctors=doctors
    )
...
def edit_mlef(case_id):
        ...
        try:
            execute_query(
                ...
                UPDATE mlef_records SET
                    ...
                    weapon_other = %s,
                    category_of_hurt = %s,
                    is_life_endangering = %s,
                    under_influence_of_alcohol = %s,
                    alcohol_smelling = %s,
                    alcohol_consumed = %s,
                    drugs_consumed = %s,
                    drugs_under_influence = %s,
                    alcohol_drugs_negative = %s,
                    accompanying_officer_info = %s,
                    sa_history = %s,
                    sa_vaginal_penetration = %s,
                    sa_anal_penetration = %s,
                    sa_inter_labial_penetration = %s,
                    mlef_investigations = %s,
                    mlef_referrals = %s,
                    mlef_other_opinions = %s
            ...
            """,
                (
                    ...
                    1 if request.form.get("has_gunshot") else 0,
                    1 if request.form.get("has_cut") else 0,
                    1 if request.form.get("has_no_injury") else 0,
                    1 if request.form.get("has_bite") else 0,
                    1 if request.form.get("has_dislocation") else 0,
                    1 if request.form.get("has_explosive_inj") else 0,
                    request.form.get("other_injuries"),
                    request.form.get("internal_injuries"),
                    1 if request.form.get("weapon_sharp") else 0,
                    1 if request.form.get("weapon_blunt") else 0,
                    1 if request.form.get("weapon_firearm") else 0,
                    1 if request.form.get("weapon_explosive") else 0,
                    request.form.get("weapon_other"),
                    request.form.get("category_of_hurt") or None,
                    1 if request.form.get("is_life_endangering") else 0,
                    1 if request.form.get("under_influence_of_alcohol") else 0,
                    1 if request.form.get("alcohol_smelling") else 0,
                    1 if request.form.get("alcohol_consumed") else 0,
                    1 if request.form.get("drugs_consumed") else 0,
                    1 if request.form.get("drugs_under_influence") else 0,
                    1 if request.form.get("alcohol_drugs_negative") else 0,
                    request.form.get("accompanying_officer_info"),
                    request.form.get("sa_history"),
                    1 if request.form.get("sa_vaginal_penetration") else 0,
                    1 if request.form.get("sa_anal_penetration") else 0,
                    1 if request.form.get("sa_inter_labial_penetration") else 0,
                    request.form.get("mlef_investigations"),
                    request.form.get("mlef_referrals"),
                    request.form.get("mlef_other_opinions"),
                    case_id,
                ),
            )
    ...
    return render_template(
        "clinical/mlef_form.html", mlef=mlef, case=case, case_id=case_id
    )
...
def update_mlr_status(case_id):
    ...
    try:
        ...
        existing = execute_query(
            "SELECT mlr_id FROM mlr_documents WHERE clinical_case_id = %s",
            (case_id,),
            fetch_all=False,
        )
        if existing:
...
def upload_photo(case_id):
    ...
    try:
        ...
        case_dir = os.path.join(
            current_app.config["UPLOADS_DIR"], f"clinical_{case_id}"
        )
...
def referrals(case_id):
    ...
    case = execute_query(
        "SELECT reference_no FROM clinical_examinations WHERE case_id = %s",
        (case_id,),
        fetch_all=False,
    )
    referrals_list = execute_query(
        "SELECT * FROM consultation_referrals WHERE clinical_case_id = %s", (case_id,)
    )
    return render_template(
        "clinical/referrals.html", case=case, case_id=case_id, referrals=referrals_list
    )
