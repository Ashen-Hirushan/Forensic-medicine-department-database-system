...
def delete_user_profile(target_user_id):
    ...
    target_user = execute_query(
        ...
        (target_user_id,), fetch_all=False
    )
    ...
    try:
        ...
        if staff and staff.get('staff_id'):
            ...
            execute_query("DELETE FROM medical_officers WHERE staff_id = %s", (staff_id,))
            execute_query("DELETE FROM staff_directory WHERE staff_id = %s", (staff_id,))
        ...
        execute_query("SET FOREIGN_KEY_CHECKS = 1;") # Ensure it gets turned back on in case of error
