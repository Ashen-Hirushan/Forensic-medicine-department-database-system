...
def list_autopsy():
    ...
    return render_template('autopsy/list.html', cases=cases, doctors=doctors,
...
def detail_autopsy(case_id):
    ...
    # PMR draft
    pmr = execute_query(
        "SELECT * FROM pmr_drafts WHERE autopsy_case_id = %s", (case_id,), fetch_all=False
    )
    
    # Death certificate / Cause of death
    cod = execute_query(
        "SELECT * FROM death_certificates WHERE autopsy_case_id = %s", (case_id,), fetch_all=False
    )
...
def edit_cod(case_id):
    if request.method == 'POST':
        try:
            existing = execute_query(
                "SELECT cod_id FROM death_certificates WHERE autopsy_case_id = %s", (case_id,), fetch_all=False
            )
            if existing:
                execute_query("""
    ...
    case = execute_query(
        "SELECT pm_serial_no FROM postmortem_investigations WHERE case_id = %s", (case_id,), fetch_all=False
    )
...
def update_pmr_status(case_id):
    ...
    try:
        existing = execute_query(
            "SELECT pmr_id FROM pmr_drafts WHERE autopsy_case_id = %s", (case_id,), fetch_all=False
        )
        if existing:
            execute_query(
